    ...
    public static func handleNewInstance<T:MarcaClass>(instance:MarcaClass?, kType:T.Type, instanceLabel:String)throws {
        if let S = kType as? Singleton.Type{
            if let si = S.getStaticInstance(){
                ...
                throw MarcaError(message: ConstantsEnum.SINGLETON_EXISTS, methodName:#function, className:String(describing:type(of:instance)), instanceLabel:instanceLabel)
            }else{
        ...
        if let i = instance{
            ...
            print("required init throwing MarcaError : NO INSTANCE EXISTS!!")
            throw MarcaError(message: ConstantsEnum.ERROR_CLASS_FACTORY, methodName:#function, className:"NONE", instanceLabel:instanceLabel)
        }
    }
}
...
class MarcaClass:Marca{
    ...
    deinit { print("DEINIT : Singleton instanceLabel:\(getInstanceLabel()) \(ConstantsEnum.WARN_MULTIPLE_INSTANCES_NOT_ALLOWED)") }
}
